...
const Login = ({ closeLoginForm }) => {
  ...
  const toggleForm = () => {
    ...
    setShowPasswordWarning(false); // Reset the warning when toggling the form
  };
  ...
  const handleRegister = (email, password) => {
    createUserWithEmailAndPassword(auth, email, password)
      .then((userCredential) => {
        ...
        setIsLoggedIn(true);
        closeLoginForm(); // Close the form on successful registration
      })
      .catch((error) => {
    ...
    if (forgotPassword) {
      ...
      signInWithEmailAndPassword(auth, email, password)
        .then((userCredential) => {
          const user = userCredential.user;
          setIsLoggedIn(true);
          closeLoginForm(); // Close the form on successful login
        })
        .catch((error) => {
          const errorCode = error.code;
          const errorMessage = error.message;
          console.error(errorCode, errorMessage);
          setShowPasswordWarning(true); // Show the warning for incorrect password
        });
    }
  };
          ...
          <>
            <form
              onSubmit={(e) => {
                ...
                handleLogin(email, ''); // Continue with handleLogin function
              }}
            >
